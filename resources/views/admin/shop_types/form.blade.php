...
@section('title', empty($shopType) ? 'Shop type create' : 'Shop type edit') 
@section('css')
<style>
    .image-manager-item{
        /* flex: 0 0 115px; */
                ...
                <div class="card">
                    <div class="card-body row">
                        <div class="col-md-6 offset-md-3 col-12">
                            <form id="form" action="{{ !empty($shopType) ? route('shop-types.update', $shopType->id) : route('shop-types.store') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @if(!empty($shopType))<input type="hidden" name="_method" value="PUT">@endif
                                <div class="row form-group">
                                    <div class="col-6 col-md-4 text-center mx-auto image-manager-box">
                                        ...
                                        @else
                                            <div class="image-manager-item position-relative">
                                                <div class="image-manager-content__upload">
                                                    <img src="{{ $shopType->image }}" class="image-manager-image" id="image-preview">
                                                </div>
                                ...
                                <div class="form-group">
                                    <label for="name"><span class="text-red">*</span> {{ __('ชื่อ') }}</label>
                                    <input type="text" class="form-control" name="name" id="name" value="{{ !empty($shopType) ? $shopType->name : '' }}">
                                </div>
                                <div class="row">
                                    <div class="col-12 text-center mt-5">
                                        <button type="submit" class="btn btn-success bg-green mr-4">บันทึกข้อมูล</button>
                                        <a href="{{ route('shop-types.index') }}" class="btn btn-default">ย้อนกลับ</a>
                                    </div>
                                </div>
                            </form>
                        </div>
    ...
    $('#form').ajaxForm({
        ...
        success: function (res) {
            Swal.fire({
                ...
                window.location.href = '{{ route("shop-types.index") }}';
            });
        },
