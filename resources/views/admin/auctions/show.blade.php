                ...
                <div class="card auction-detail">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <p class="col-auto">{{date('d/m/Y H:i', strtotime($auction->created_at))}}</p>
                        </div>
                        <div class="row">
                            <p class="col-6">วันที่จบการประมูล:</p>
                            <p class="col-auto">{{date('d/m/Y H:i', strtotime($auction->expired_at))}}</p>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $('#btn-bid').on('click',function(e){
        var __url = '{{ route("auctions.bidding","__id") }}';
        ...
        $.ajax({
            ...
            // $('#text-lastes-price').html(pricceFormat(data.latest_price));
            ...
            // $('#amount').val(pricceFormat(amount));
            Swal.fire({
                icon:'success',
    ...
    auctionRef.on('value', (snapshot) =>{
        var __url = '{{ route("auctions.get","__id") }}';
        __url = __url.replace('__id', {{ $auction->id }})
        $.ajax({
            url: __url,
            method: "GET",
    ...
    // var detailTable;
    // detailTable = $('#auction-details-table').DataTable({
    //     processing: true,
    //     serverSide: true,
    //     ajax:{
    //         url:"{!! route('auctions.details.data') !!}",
    //         data: function (d) {
    //             d.sort_id = 'desc';
    //         }
    //     },
    ...
    //                 return full.created_by_user.first_name+' '+full.created_by_user.last_name;
    //             }
    //         },
    //         {
    //             targets:2,
    ...
    //         if (!$('#auction-details-table').parent().hasClass("table-responsive")) {
    //             $('#auction-details-table').wrap("<div class='table-responsive'></div>");
    //         }
    //         loader.close();
    //     },
    // });
