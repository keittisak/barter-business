...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-md-6">
                <h1>ภาพสไลด์{{ (Request('after_login') == 'y') ? 'หลัง Login' : ''   }}</h1>
            </div>
            <div class="col-md-6 text-right">
                <a href="{{ route('branners.create',['after_login' => Request('after_login')]) }}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> เพิ่มภาพสไลด์{{ (Request('after_login') == 'y') ? 'หลัง Login' : ''   }}</a>
            </div>
        </div>
    </div>
</section>
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">ภาพสไลด์ทั้งหมด</h3>
            </div>
...
<script>
    ...
    table = $dt.DataTable({
        ...
        serverSide: true,
        ajax:{
            url:"{!! route('branners.data') !!}",
            data: function (d) {
                d.after_login = '{{ Request('after_login') ?? "n" }}'
            }
        ...
        columnDefs : [
            ...
            },
            {
                targets:4,
                render: function (data, type, full, meta){
                    var __url_edit = "{{ route('branners.edit','__id') }}";
                    ...
                    <a href="${__url_edit}?after_login={{Request("after_login")}}" class="btn btn-warning btn-sm"><i class="far fa-edit"></i></a>
    ...
    $(document).on('click', '.btn-delete', function(e){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var __url = "{{ route('branners.delete','__id') }}";
                ...
                $.ajax({
                    ...
                    Swal.fire(`ลบภาพสไลด์เรียบร้อย`, ``, `success`);
                    table.draw();
                    loader.close();
