...
@section('title','Point transfer success') 
@section('css')
<style>
    .icon-transfer-down {
        display: block;
            ...
            <div class="card-body">
                ...
                <div class="user-block d-inline-block w-100 mt-4">
                    ...
                    <div class="avatars rounded-circle avatars-sm border-green  float-left mr-4 ">
                        <div class="avatars-one" style="background-image: url({{ ( empty($data->created_by_user) ||  empty($data->created_by_user->image) ) ? asset('assets/images/img_profile_default.jpg') : $data->created_by_user->image }})"></div>
                    </div>
                    <span class="username">{{ empty($data->created_by_user) ? 'BA system' : $data->created_by_user->first_name.' '.$data->created_by_user->last_name }}</span>
                    <span class="description">{!! empty($data->created_by_user) ? '&nbsp;' : 'รหัสผู้ซื้อ  #'.$data->created_by_user->code !!}</span>
                </div>
                ...
                <div class="user-block d-inline-block w-100">
                    <div class="avatars rounded-circle avatars-sm border-green  float-left mr-4 ">
                        <div class="avatars-one" style="background-image: url({{ empty($data->transferred_to_user->image) ? asset('assets/images/img_profile_default.jpg') : $data->transferred_to_user->image }})"></div>
                    </div>
                    <span class="username">{{ empty($data->transferred_to_user) ? 'XXXXX' : $data->transferred_to_user->first_name.' '.$data->transferred_to_user->last_name }}</span>
                    <span class="description">รหัสผู้ขาย  #{{ empty($data->transferred_to_user) ? 'XXXXX' : $data->transferred_to_user->code }}</span>
                </div>
